...
public class xx1856Controller {
    ...
    @PutMapping("18xx/lateDestination/{game}/{corp}")
    @ResponseBody
    Board lateDestination(@PathVariable String game, @PathVariable String corp) {
        return findGame(game).addMove(DESTINATION_REACHED, "", corp, 1, "");
    }
    ...
    @PutMapping("18xx/tradeIn/{game}/{corp}/{size}")
    @ResponseBody
    Board tradeIn(@PathVariable String game, @PathVariable String corp,
                  @PathVariable int size) {
        return findGame(game).addMove(TRADE_IN_TRAIN, "", corp, size, "");
    }
    ...
    @ResponseBody
    Board buyPoolTrain(@PathVariable String game, @PathVariable String corp,
                       @PathVariable int train) {
        return findGame(game).addMove(BUY_POOL_TRAIN, "", corp, train, "");
    }
    ...
    Board endOpTurn(@PathVariable String game, @PathVariable String corp) {
        Game g = findGame(game);
        return g.addMove(END_OP_TURN, "", corp, 0, g.getBoard().getActivity());
    }

    @PutMapping("18xx/buyPriv/{game}/{corp}/{priv}/{player}/{price}")
    @ResponseBody
    Board buyPriv(@PathVariable String game, @PathVariable String corp,
